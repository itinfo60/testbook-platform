const fs = require('fs');
const file =
  '/Users/balveerchoudhary/testbook-platform/client/src/features/home/pages/EduPortalHome.jsx';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Mock demo classes from courses for now
...
content = content.replace(
  'EDU_PORTAL_DATA.trendingAlerts.map((alert)',
...
content = content.replace(
  'EDU_PORTAL_DATA.examCategories.map((category)',
...
content = content.replace(
  '{category.subtitle}',
  "{category.description || category.subtitle || 'Explore our courses'}"
);
...
content = content.replace(
  'search=${encodeURIComponent(exam.name)}',
  "search=${encodeURIComponent(exam.name || exam.title || '')}"
);

content = content.replace(
  'EDU_PORTAL_DATA.freeResources.map((res)',
...
content = content.replace(
  'EDU_PORTAL_DATA.paidCoursesHighlights.map((course)',
...
content = content.replace(
  ...
  '{course.enrolledCount || course.studentsCount || 0}'
);
content = content.replace(
  '{course.category}',
...
content = content.replace(
  'EDU_PORTAL_DATA.demoLectures.map((demo)',
...
content = content.replace(
  '{demo.teacher}',
  "{demo.teacher?.name || demo.teacher || 'Expert Faculty'}"
);
content = content.replace('{demo.views}', "{demo.views || '1K+'}");

const testSeriesRegex = /\\[\\s*\\{[\\s\\S]*?\\}\\s*\\]\\.map\\(\\(feature,\\s*idx\\)\\s*=>/m;
content = content.replace(
  /\[\s*\{\s*title:\s*'📋 Subject-Wise Tests'[\s\S]*?\]\.map\(\(feature,\s*idx\)\s*=>/m,
...
content = content.replace(
  'EDU_PORTAL_DATA.blogs.map((blog)',
...
content = content.replace(
  '{blog.snippet}',
  "{blog.snippet || blog.excerpt || blog.content?.substring(0, 100) || 'Read more about this article...'}"
);
content = content.replace(
  '{blog.date}',
