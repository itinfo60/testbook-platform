...
const seedData = async () => {
  try {
    ...
    // 1. Create distinct users
    console.log('Creating Admin, Teacher, and Student users...');
    ...
    // 7. Seed Student Interactions
    console.log('Seeding student interactions (Enrollments & Test Attempts)...');

    // Enroll Student in Patwari Course
    const patwariCourse = await Course.findOne({
      slug: 'target-patwari-special-foundation-batch-2026',
    });
    if (patwariCourse) {
      await Enrollment.create({
        user: studentId,
        course: patwariCourse._id,
        progress: [],
      ...
      await User.updateOne({ _id: studentId }, { $inc: { enrolledCourses: 1 }, streak: 7 });
    }

    // Enroll Student in Test Series
    const pyqTestSeries = await TestSeries.findOne({ slug: 'rpsc-eo-ro-pyq-series' });
    if (pyqTestSeries) {
      // Find a test inside that series and create a completed attempt
      const test = await Test.findOne({ testSeries: pyqTestSeries._id });
      if (test) {
        await Enrollment.create({
          user: studentId,
          test: test._id,
        ...
        await User.updateOne({ _id: studentId }, { $inc: { totalTestsTaken: 1 } });
      }
    }
