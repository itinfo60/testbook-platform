  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/users')} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{isEdit ? 'Edit User' : 'Create User'}</h2>
          <p className="text-gray-500 dark:text-gray-400">{isEdit ? 'Update user details' : 'Add a new platform user'}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        <div>
          ...
          <input type="text" value={form.name} onChange={handleChange('name')} className="input-field" required />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="password" value={form.password} onChange={handleChange('password')} className="input-field" {...(!isEdit && { required: true })} minLength={6} />
        </div>
