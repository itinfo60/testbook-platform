...
import { createExamCategory, updateExamCategory, fetchExamCategoryById, clearSelected } from '@/features/examcategory/examCategorySlice';
...
export default function ExamCategoryForm() {
  ...
  if (isEdit && loading && !selected) return <div className="flex justify-center py-20"><LoadingSpinner size="lg" /></div>;

  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/exam-categories')} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{isEdit ? 'Edit Exam Category' : 'Create Exam Category'}</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" className="btn-primary gap-2"><Save className="w-4 h-4" /> {isEdit ? 'Update' : 'Create'}</button>
          <button type="button" onClick={() => navigate('/exam-categories')} className="btn-secondary">Cancel</button>
        </div>
      </form>
    </div>
  );
