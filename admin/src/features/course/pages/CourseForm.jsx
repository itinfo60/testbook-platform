        ...
        {currentStep === 4 && (
          <div className="animate-fade-in space-y-6">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm space-y-4">
              ...
              {/* Demo / Sample Class Video Upload / URL */}
              <FileUploadInput
                label="Sample / Demo Preview Video"
                value={form.demoVideoUrl}
                onChange={(url) => setField('demoVideoUrl', url)}
                type="video"
                folder="course-demos"
                placeholder="https://... (MP4, WebM or YouTube link)"
                hint="Upload demo video to Supabase Storage or enter public video link. Displayed in student preview player."
              />

              {/* Course Poster / Thumbnail Upload / URL */}
              <FileUploadInput
                label="Course Poster / Thumbnail"
                value={form.thumbnailUrl}
                onChange={(url) => setField('thumbnailUrl', url)}
                type="image"
                folder="course-thumbnails"
                placeholder="https://..."
                hint="Upload course cover image to Supabase or enter an image URL."
              />
...
function LessonEditor({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3.5 space-y-3 shadow-xs">
      ...
      {lesson.type === 'video' && (
        <FileUploadInput
          label="Lesson Video (MP4 / Stream / YouTube)"
          value={lesson.videoUrl}
          onChange={(url) => onUpdate('videoUrl', url)}
          type="video"
          folder="lesson-videos"
          placeholder="Upload video or paste YouTube/Vimeo/MP4 link"
        />
      )}
      ...
      <div>
        ...
        >
          ...
          Attached Study Notes & Documents ({lesson.resources?.length || 0})
          {showResources ? (
            ...
            {(lesson.resources || []).map((res, ri) => (
              ...
              >
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 text-xs bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white"
                />
                <FileUploadInput
                  label="Document / PDF File"
                  value={res.url}
                  onChange={(url) => onUpdateResource(ri, 'url', url)}
                  type="document"
                  folder="lesson-notes"
