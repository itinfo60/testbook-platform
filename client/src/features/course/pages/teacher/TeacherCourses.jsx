  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-100 dark:border-dark-700">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">My Courses</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage and publish your educational content</p>
        </div>
        <Link to="/teacher/courses/new" className="group relative inline-flex items-center justify-center px-6 py-2.5 text-sm font-semibold text-white transition-all duration-200 bg-primary-600 font-pj rounded-xl hover:bg-primary-700 shadow-lg shadow-primary-500/30 hover:-translate-y-0.5">
          <HiPlus className="mr-2 h-4 w-4" /> Create Course
        </Link>
      </div>
      ...
      {teacherCourses.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center bg-white dark:bg-dark-800 rounded-3xl border border-slate-100 dark:border-dark-700 shadow-sm">
          ...
          <p className="text-slate-500 max-w-md mb-6">Create your first course to start teaching and sharing your knowledge.</p>
              ...
              <div className="h-48 w-full bg-slate-100 dark:bg-dark-900 relative overflow-hidden">
                {(course.thumbnail?.url || course.thumbnail) ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-primary-500/20 to-primary-700/20 dark:from-primary-900/40 dark:to-primary-800/40">
                    ...
                    <span className="text-xs font-medium text-primary-600 dark:text-primary-400">No Cover Image</span>
                  </div>
                )}
                
                {/* Status Badge overlay */}
                <div className="absolute top-4 right-4">
                  <span className={`px-3 py-1.5 text-xs font-bold rounded-xl shadow-sm backdrop-blur-md ${course.isPublished ? 'bg-emerald-500/90 text-white' : 'bg-amber-500/90 text-white'}`}>
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                ...
                <div className="mt-5 flex items-center justify-between text-sm border-t border-slate-50 dark:border-dark-700/50 pt-4">
                  <div className="flex flex-col">
                    <span className="text-slate-400 text-xs uppercase tracking-wider font-semibold">Lessons</span>
                    <span className="text-slate-800 dark:text-slate-200 font-bold mt-0.5">{course.totalLessons || 0}</span>
                  </div>
                  <div className="h-8 w-px bg-slate-200 dark:bg-dark-700"></div>
                  <div className="flex flex-col">
                    <span className="text-slate-400 text-xs uppercase tracking-wider font-semibold">Students</span>
                    <span className="text-slate-800 dark:text-slate-200 font-bold mt-0.5">{course.enrollmentCount || 0}</span>
                  </div>
                </div>
              </div>
            </div>
