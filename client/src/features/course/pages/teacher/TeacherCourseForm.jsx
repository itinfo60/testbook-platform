...
import {
  HiPlus, HiTrash, HiChevronDown, HiChevronUp,
...
export default function TeacherCourseForm() {
  ...
  useEffect(() => {
    if (isEdit && currentCourse && currentCourse._id === id) {
      ...
      const open = {};
      (currentCourse.sections || []).forEach((_, i) => { open[i] = true; });
  ...
  const setArrayItem = (key, index, value) =>
    setField(key, form[key].map((item, i) => i === index ? value : item));

  const addArrayItem = (key, empty = '') =>
    setField(key, [...form[key], empty]);
  ...
  const updateSection = (si, key, value) =>
    setField('sections', form.sections.map((s, i) => i === si ? { ...s, [key]: value } : s));

  const removeSection = (si) =>
    setField('sections', form.sections.filter((_, i) => i !== si));
  ...
  const removeLesson = (si, li) =>
    updateSection(si, 'lessons', form.sections[si].lessons.filter((_, i) => i !== li));
    ...
    <div className="max-w-4xl w-full">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="card p-6 space-y-4">
          <h3 className="font-semibold text-dark-900 dark:text-white border-b border-dark-100 dark:border-dark-700 pb-3">Basic Info</h3>

          <Input label="Course Title *" value={form.title} onChange={e => setField('title', e.target.value)} placeholder="e.g. Complete Python Bootcamp" />
          ...
          <div>
            <label className="block text-sm font-medium text-dark-700 dark:text-dark-300 mb-1.5">Short Description</label>
            <textarea
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.level} onChange={e => setField('level', e.target.value)} className="input-field">
        ...
        <div className="card p-6 space-y-3">
          <h3 className="font-semibold text-dark-900 dark:text-white border-b border-dark-100 dark:border-dark-700 pb-3">What You'll Learn</h3>
          {form.whatYouLearn.map((item, i) => (
            <div key={i} className="flex gap-2">
              ...
              {form.whatYouLearn.length > 1 && (
                <button type="button" onClick={() => removeArrayItem('whatYouLearn', i)} className="p-2 text-red-400 hover:text-red-600">
                  <HiTrash className="h-4 w-4" />
                </button>
              )}
            </div>
          ))}
          <button type="button" onClick={() => addArrayItem('whatYouLearn', '')} className="text-sm text-primary-600 dark:text-primary-400 hover:underline flex items-center gap-1">
        ...
        {/* ── Requirements ────────────────────────────────────────── */}
        <div className="card p-6 space-y-3">
          <h3 className="font-semibold text-dark-900 dark:text-white border-b border-dark-100 dark:border-dark-700 pb-3">Requirements</h3>
          {form.requirements.map((item, i) => (
            <div key={i} className="flex gap-2">
              ...
              {form.requirements.length > 1 && (
                <button type="button" onClick={() => removeArrayItem('requirements', i)} className="p-2 text-red-400 hover:text-red-600">
                  <HiTrash className="h-4 w-4" />
                </button>
              )}
            </div>
          ))}
          <button type="button" onClick={() => addArrayItem('requirements', '')} className="text-sm text-primary-600 dark:text-primary-400 hover:underline flex items-center gap-1">
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between border-b border-dark-100 dark:border-dark-700 pb-3 mb-4">
            <div>
              ...
              <p className="text-xs text-dark-400 mt-0.5">{form.sections.length} sections · {totalLessons} lessons</p>
            </div>
            <button type="button" onClick={addSection} className="btn-outline text-sm flex items-center gap-1.5">
          ...
          {form.sections.length === 0 ? (
            <div className="text-center py-10 text-dark-400">
              ...
              <p className="text-sm">No sections yet. Add a section to start building your curriculum.</p>
...
function SectionEditor({ section, sectionIndex, isOpen, onToggle, onUpdateSection, onRemoveSection, onAddLesson, onUpdateLesson, onRemoveLesson, onAddResource, onUpdateResource, onRemoveResource }) {
  return (
    <div className="border border-dark-200 dark:border-dark-700 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-dark-50 dark:bg-dark-800/60">
        <button type="button" onClick={onToggle} className="flex-1 flex items-center gap-3 text-left">
          {isOpen ? <HiChevronUp className="h-4 w-4 text-dark-400 flex-shrink-0" /> : <HiChevronDown className="h-4 w-4 text-dark-400 flex-shrink-0" />}
          <div className="flex-1 min-w-0">
            <span className="text-xs text-dark-400 font-medium">Section {sectionIndex + 1}</span>
            <p className="text-sm font-semibold text-dark-900 dark:text-white truncate">{section.title || 'Untitled Section'}</p>
          </div>
          <span className="text-xs text-dark-400 flex-shrink-0">{section.lessons.length} lessons</span>
        </button>
        <button type="button" onClick={onRemoveSection} className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors flex-shrink-0">
          <HiTrash className="h-4 w-4" />
        </button>
      </div>
      ...
      {isOpen && (
        <div className="p-4 space-y-4">
          ...
          <button type="button" onClick={onAddLesson} className="w-full py-2.5 border-2 border-dashed border-dark-200 dark:border-dark-600 rounded-xl text-sm text-dark-400 hover:border-primary-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors flex items-center justify-center gap-2">
...
function LessonEditor({ lesson, lessonIndex, onUpdate, onRemove, onAddResource, onUpdateResource, onRemoveResource }) {
    ...
    <div className="bg-white dark:bg-dark-900 border border-dark-100 dark:border-dark-700 rounded-xl p-4 space-y-3">
      ...
      <div className="flex items-center gap-2">
        <div className={`h-7 w-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
          lesson.type === 'video' ? 'bg-primary-100 dark:bg-primary-900/30 text-primary-600' :
          lesson.type === 'quiz' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' :
        ...
        <button type="button" onClick={onRemove} className="p-1 text-red-400 hover:text-red-600 rounded-lg transition-colors">
      ...
      {(lesson.type === 'text' || lesson.type === 'quiz') && (
        <textarea
          ...
          placeholder={lesson.type === 'quiz' ? 'Quiz instructions / description' : 'Lesson content (markdown supported)'}
      ...
      <div>
        ...
        >
          ...
          {showResources ? <HiChevronUp className="h-3 w-3" /> : <HiChevronDown className="h-3 w-3" />}
        </button>
