...
const VideoPlayer = forwardRef(({ url, onProgress, onComplete }, ref) => {
  ...
  // Masks active by default so it starts covered while video is paused/unstarted
  ...
  // Enforce turning off captions on YouTube iframe
  ...
  /**
   * Hover & Visibility State:
   * - If video is PAUSED -> solid black top/bottom ribbons stay ALWAYS active.
   * - If cursor is inside the player window -> ribbons stay ALWAYS active (NO TIMEOUT while hovered).
   * - If video is PLAYING and cursor LEAVES the player -> wait 5 seconds, then hide.
   */
  ...
  const handleMouseLeave = () => {
    ...
    // Only hide after 5 seconds IF the video is actively playing
        ...
        if (data.event === 'infoDelivery' && data.info) {
          ...
          if (typeof data.info.playerState === 'number') {
            ...
            if (state === 1) {
              ...
              // PAUSED -> ALWAYS keep black ribbons visible
              setIsPlaying(false);
  ...
  // Window-level mouse position tracking for Fullscreen and normal mode
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <iframe
        ref={iframeRef}
        src={`https://www.youtube.com/embed/${videoId}?enablejsapi=1&color=white&controls=1&rel=0&modestbranding=1&fs=0&iv_load_policy=3&cc_load_policy=0&cc_lang_pref=none&hl=en&playsinline=1`}
      ...
      {/* 2. Top Solid Black Ribbon (Only covers the top header strip: title, channel & gear) */}
      <div
      ...
      {/* 3. Bottom Solid Black Ribbon (Only covers the bottom footer strip: share & YouTube watermark) */}
      <div
      ...
      {/* 4. Custom Fullscreen Container Button (Keeps black ribbons active in fullscreen) */}
      <button
        onClick={toggleFullscreen}
        title={isFullscreen ? 'Exit Fullscreen' : 'Fullscreen'}
        className={`absolute bottom-2 right-2.5 w-9 h-9 bg-neutral-900 hover:bg-blue-600 border border-white/20 hover:border-blue-600 text-white rounded-lg flex items-center justify-center z-20 ${
          activeMasks ? 'block opacity-100' : 'hidden opacity-0 pointer-events-none'
        }`}
      >
        {isFullscreen ? (
