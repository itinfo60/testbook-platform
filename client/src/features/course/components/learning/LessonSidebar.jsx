...
import { HiCheck, HiDocument, HiLockClosed, HiPlay, HiChevronDown, HiPencilAlt } from 'react-icons/hi';
...
export default function LessonSidebar({ sections = [], currentLessonId, completedLessonIds = [], onSelectLesson, totalCompleted, totalLessons }) {
            ...
            <div key={section._id || si}>
              ...
              >
                <div className="flex-1 min-w-0 pr-2">
                  <p className="text-sm font-semibold text-dark-800 dark:text-dark-200 truncate">{section.title}</p>
                  <p className="text-xs text-dark-400 mt-0.5">
                    {sectionCompleted}/{section.lessons.length} · {section.lessons.reduce((s, l) => s + (l.duration || 0), 0) > 0 ? formatDuration(section.lessons.reduce((s, l) => s + (l.duration || 0), 0)) : ''}
                  </p>
                </div>
                <HiChevronDown className={`h-4 w-4 text-dark-400 flex-shrink-0 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
              </button>
                ...
                return (
                  ...
                  >
                    <div className={`flex-shrink-0 h-7 w-7 rounded-lg flex items-center justify-center ${
                      isDone ? 'bg-secondary-100 dark:bg-secondary-900/40' :
                      ...
                      {isDone
                        ...
                        : typeIcon(lesson.type, `h-3.5 w-3.5 ${isActive ? 'text-primary-500' : 'text-dark-400'}`)
                      }
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm truncate leading-snug ${
                        isActive ? 'font-medium text-primary-600 dark:text-primary-400' :
                        isDone ? 'text-dark-500 dark:text-dark-400' :
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5 text-xs text-dark-400">
                        ...
                        {lesson.duration > 0 && <><span>·</span><span>{formatDuration(lesson.duration)}</span></>}
                        {lesson.isFree && <span className="text-secondary-500 font-medium">Free</span>}
                      </div>
                    </div>
                  </button>
                );
