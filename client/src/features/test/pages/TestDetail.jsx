  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      <div className="card p-4 sm:p-6 lg:p-8">
        ...
        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-dark-900 dark:text-white mb-3 leading-tight">{test.title}</h1>
        {test.description && <p className="text-dark-600 dark:text-dark-400 mb-6 text-sm sm:text-base">{test.description}</p>}

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="card p-3 sm:p-4 text-center bg-dark-50 dark:bg-dark-800/50">
            <HiClipboardList className="h-5 w-5 sm:h-6 sm:w-6 text-primary-500 mx-auto mb-1" />
            <div className="text-base sm:text-lg font-bold text-dark-900 dark:text-white">{totalQ}</div>
            <div className="text-xs text-dark-400">Questions</div>
          </div>
          <div className="card p-3 sm:p-4 text-center bg-dark-50 dark:bg-dark-800/50">
            <HiClock className="h-5 w-5 sm:h-6 sm:w-6 text-accent-500 mx-auto mb-1" />
            <div className="text-base sm:text-lg font-bold text-dark-900 dark:text-white">{test.duration || 60}</div>
            <div className="text-xs text-dark-400">Minutes</div>
          </div>
          <div className="card p-3 sm:p-4 text-center bg-dark-50 dark:bg-dark-800/50">
            <HiUsers className="h-5 w-5 sm:h-6 sm:w-6 text-secondary-500 mx-auto mb-1" />
            <div className="text-base sm:text-lg font-bold text-dark-900 dark:text-white">{attemptCount}</div>
          ...
          <div className="card p-3 sm:p-4 text-center bg-dark-50 dark:bg-dark-800/50">
            ...
            <div className="text-base sm:text-lg font-bold text-dark-900 dark:text-white">{test.totalMarks || totalQ}</div>
            <div className="text-xs text-dark-400">Total Marks</div>
          </div>
        </div>

        {/* Instructions */}
        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-xl p-4 sm:p-5 mb-6">
          <h3 className="font-semibold text-amber-800 dark:text-amber-400 mb-2 text-sm sm:text-base">📋 Instructions</h3>
          <ul className="text-xs sm:text-sm text-amber-700 dark:text-amber-300 space-y-1.5">
