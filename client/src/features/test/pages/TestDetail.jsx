...
export default function TestDetail() {
  ...
  const isButtonDisabled = hasReachedLimit && !activeAttempt && !isPaidAndUnpurchased;

  let buttonText = 'Start Test';
  if (isPaidAndUnpurchased) {
    buttonText = `Buy Test - ₹${test.price}`;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      <div className="card p-4 sm:p-6 lg:p-8">
        ...
        {test.description && (
          <p className="text-dark-600 dark:text-dark-400 mb-6 text-sm sm:text-base">
            {test.description}
          </p>
        )}
        ...
        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-xl p-4 sm:p-5 mb-6">
          <h3 className="font-semibold text-amber-800 dark:text-amber-400 mb-2 text-sm sm:text-base">
            📋 Instructions
          </h3>
          <ul className="text-xs sm:text-sm text-amber-700 dark:text-amber-300 space-y-1.5">
            <li>
              • This test has {totalQ} questions to be completed in {test.duration || 60} minutes.
            </li>
            <li>• Each correct answer carries {test.marksPerQuestion || 1} mark(s).</li>
            {test.negativeMarking && (
              <li>• Negative marking: {test.negativeMarks || 0.25} marks per wrong answer.</li>
            )}
            {test.maxAttempts > 0 && (
              <li>
                • Maximum attempts allowed: {test.maxAttempts} (Attempts completed: {attemptCount}).
              </li>
            )}
        ...
        <div className="flex gap-3 justify-center">
          <Button
            variant="primary"
            size="lg"
            icon={isPaidAndUnpurchased || (hasReachedLimit && !activeAttempt) ? null : HiPlay}
