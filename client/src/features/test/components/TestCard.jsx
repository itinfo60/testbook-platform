    ...
    <Link to={`/tests/${_id}`} className="card-hover p-4 sm:p-5 group">
      <div className="flex items-start justify-between mb-3 gap-2">
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 min-w-0">
          <span className="badge-primary truncate max-w-[120px] sm:max-w-none">{typeof category === 'object' ? category?.name : category || 'General'}</span>
      ...
      {description && (
        <p className="text-xs sm:text-sm text-dark-500 dark:text-dark-400 line-clamp-2 mb-3">{description}</p>
      )}

      <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs text-dark-400 mb-3 sm:mb-4">
        <span className="flex items-center gap-1"><HiClipboardList className="h-3.5 w-3.5" />{qCount} Qs</span>
        <span className="flex items-center gap-1"><HiClock className="h-3.5 w-3.5" />{duration || 60} min</span>
        <span className="flex items-center gap-1"><HiUsers className="h-3.5 w-3.5" />{totalAttempts || attemptCount || 0}</span>
      </div>
