...
export default function TeacherRevenue() {
  ...
  const stats = [
    { label: 'Total Earned', value: `₹${totalRevenue.toLocaleString('en-IN')}`, icon: HiCurrencyRupee, color: 'from-emerald-400 to-teal-500' },
    { label: 'Total Orders', value: totalOrders, icon: HiShoppingCart, color: 'from-blue-500 to-indigo-600' },
    { label: 'Completed', value: totalOrders, icon: HiTrendingUp, color: 'from-purple-500 to-pink-600' },
    { label: 'Avg. Order', value: `₹${avgOrder.toLocaleString('en-IN')}`, icon: HiCalendar, color: 'from-amber-400 to-orange-500' },
  ];

  // Group by course
  const groupedByCourse = Object.values(payments.reduce((acc, payment) => {
    ...
    <div className="animate-fade-in">
      ...
      <div className="bg-white dark:bg-dark-800 rounded-3xl p-6 border border-slate-100 dark:border-dark-700 shadow-sm">
        <div className="mb-6 pb-4 border-b border-slate-100 dark:border-dark-700">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Breakdown of earnings and orders by course</p>
        </div>
        ...
        {groupedByCourse.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">No revenue yet</h3>
                ...
                <div key={courseId} className="border border-slate-100 dark:border-dark-700 rounded-2xl overflow-hidden transition-all duration-300">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className={`p-2 rounded-full bg-slate-100 dark:bg-dark-700 text-slate-400 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}>
                  ...
                  {isExpanded && (
                    <div className="bg-slate-50 dark:bg-dark-900/30 border-t border-slate-100 dark:border-dark-700 animate-fade-in p-5">
                      <h5 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Payment History ({group.payments.length})</h5>
                      <div className="divide-y divide-slate-100 dark:divide-dark-700/50">
                        {group.payments.map((payment, idx) => {
                          const avatarUrl = payment.user?.avatar?.url || (typeof payment.user?.avatar === 'string' ? payment.user?.avatar : null);
                          return (
                            <div key={payment._id || idx} className="py-3 flex items-center justify-between first:pt-0 last:pb-0">
