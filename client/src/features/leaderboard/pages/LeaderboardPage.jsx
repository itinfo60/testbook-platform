  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px8 py-8">
      ...
      {userRank ? (
        ...
        <div className="card p-4 mb-6 border border-dashed border-dark-200 dark:border-dark-700 text-center">
          <p className="text-sm text-dark-400">
            Complete tests this {periodLabel.toLowerCase()} to appear on the leaderboard
          </p>
        </div>
      )}
            ...
            >
              ...
              <div className="w-8 sm:w-10 text-center flex-shrink-0">
                {i < 3 ? (
                  ...
                  <span className="text-base sm:text-lg font-bold text-dark-400">{entry.rank}</span>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-dark-900 dark:text-white truncate text-sm sm:text-base">
                  {entry.name}
                </p>
                <p className="text-xs text-dark-400">
                  {entry.testsCompleted ?? 0} {entry.testsCompleted === 1 ? 'test' : 'tests'} ·{' '}
                  {entry.avgPercentage ?? 0}% avg
                </p>
              </div>

              {/* Score */}
              <div className="text-right flex-shrink-0">
                <p className="font-bold text-dark-900 dark:text-white text-sm sm:text-base">
                  {(entry.totalPoints ?? 0).toLocaleString()}
                </p>
                <p className="text-xs text-dark-400">pts</p>
              </div>
            </div>
          ))}
        </div>
