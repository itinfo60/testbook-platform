...
beforeEach(() => { vi.resetModules(); vi.useFakeTimers(); delete window.YT; });
afterEach(() => {
  document.querySelectorAll('script[src="https://www.youtube.com/iframe_api"]').forEach((node) => node.remove());
...
it('retries a failed preexisting script instead of waiting on it forever', async () => {
  ...
  expect(document.querySelectorAll('script[src="https://www.youtube.com/iframe_api"]')).toHaveLength(2);
  window.YT = { Player: vi.fn() };
  window.onYouTubeIframeAPIReady();
