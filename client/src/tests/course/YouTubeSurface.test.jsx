...
import { getVideoSource, extractYouTubeId } from '../../features/course/components/learning/videoSource';

const mocks = vi.hoisted(() => ({ load: vi.fn(), players: [] }));
vi.mock('../../features/course/components/learning/youtubeApi', () => ({ loadYouTubeApi: mocks.load }));
...
const SDK = {
  Player: function (iframe, { events }) {
    Object.assign(this, {
      ...
      setVolume: vi.fn(), mute: vi.fn(), unMute: vi.fn(), setPlaybackRate: vi.fn(),
      getDuration: vi.fn(() => 100), getCurrentTime: vi.fn(() => 25),
...
function ready(player) { act(() => player.events.onReady({ target: player })); }
...
describe('YouTube iframe boundary', () => {
  it('sets sandbox and input restrictions before the SDK receives the frame', async () => {
    ...
    for (const [key, value] of Object.entries({ controls: '0', disablekb: '1', fs: '0', playsinline: '1', origin: location.origin })) {
  ...
  it('queues commands before ready, reports progress, and handles blocked autoplay', async () => {
    ...
    const blocked = vi.fn();
    render(<YouTubeSurface ref={ref} videoId={ID} onProgress={progress} onAutoplayBlocked={blocked} />);
    act(() => { ref.current.seekTo(42); ref.current.setVolume(0.4); ref.current.setMuted(true); });
    ...
    expect(progress).toHaveBeenLastCalledWith({ played: 0.25, playedSeconds: 25, duration: 100, loaded: 0.6 });
  ...
  it('cleans up timers and ignores stale callbacks when switching sources', async () => {
    const progress = vi.fn();
    const state = vi.fn();
    const { rerender, unmount } = render(<YouTubeSurface videoId={ID} onProgress={progress} onStateChange={state} />);
    await settle();
    ...
    state.mockClear(); progress.mockClear();
  ...
  it('survives StrictMode and reports a readiness timeout locally', async () => {
    const error = vi.fn();
    const { container } = render(<StrictMode><YouTubeSurface videoId={ID} onError={error} /></StrictMode>);
    await settle();
