  ...
  return (
    <div className={`space-y-2 ${className}`}>
      {items.map((item, index) => (
        <div key={index} className="border border-dark-100 dark:border-dark-700 rounded-xl overflow-hidden">
          <button
          ...
          >
            <div className="flex items-center gap-3">
              {item.icon && <span className="text-primary-600 dark:text-primary-400">{item.icon}</span>}
              <div>
            ...
            <HiChevronDown className={`h-5 w-5 text-dark-400 transition-transform duration-200 ${openItems.includes(index) ? 'rotate-180' : ''}`} />
          </button>
          <AnimatePresence>
