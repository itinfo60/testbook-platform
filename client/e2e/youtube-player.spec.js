...
test('blocks frame popups and top navigation, even if its script attempts them', async ({
  page,
  context,
}) => {
  ...
  const frame = page
    .frames()
    .find((candidate) => candidate.url().startsWith('https://www.youtube-nocookie.com/'));
  expect(frame).toBeTruthy();
  const result = await frame.evaluate(() => {
    let topBlocked = false;
    try {
      window.top.location.href = 'https://www.youtube.com/watch?v=W6NZfCO5SIk';
    } catch {
      topBlocked = true;
    }
...
test('fallback fullscreen escapes clipped ancestors, traps focus, and exits without remounting', async ({
  page,
}) => {
  ...
  expect(await page.evaluate(() => document.querySelector('iframe') === window.originalFrame)).toBe(
    true
  );
