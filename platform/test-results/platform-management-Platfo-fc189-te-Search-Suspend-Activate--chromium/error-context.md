...
- Name: platform-management.spec.js >> Platform Super Admin Management Features - Deep Dive >> super admin can manage institutes (Create, Search, Suspend/Activate)
- Location: e2e/platform-management.spec.js:16:3
...
Expected substring: "E2E Academy 1779887939204"
Received string:    "InstituteSubdomainPlanStatusExpiresLimitsActionsDemo Academyinstitute@demo.comdemo.testbook.comgrowthactive5/27/2027500 studentsSuspendDelete"
Timeout: 10000ms

Call log:
  - Expect "toContainText" with timeout 10000ms
  - waiting for locator('table')
    24 × locator resolved to <table class="w-full text-sm">…</table>
       - unexpected value "InstituteSubdomainPlanStatusExpiresLimitsActionsDemo Academyinstitute@demo.comdemo.testbook.comgrowthactive5/27/2027500 studentsSuspendDelete"
...
- table:
    - rowgroup:
        - row "Institute Subdomain Plan Status Expires Limits Actions":
            - columnheader "Institute"
    ...
    - rowgroup:
        - row "Demo Academy institute@demo.com demo .testbook.com growth active 5/27/2027 500 students Suspend Delete":
            - cell "Demo Academy institute@demo.com":
                - paragraph: Demo Academy
                - paragraph: institute@demo.com
            - cell "demo .testbook.com"
            - cell "growth"
            - cell "active"
            - cell "5/27/2027"
            - cell "500 students"
...
```ts
  ...
  2   | import { loginAsSuperAdmin } from './helpers/auth.js';
  3   |
  4   | test.describe('Platform Super Admin Management Features - Deep Dive', () => {
  5   |   test.beforeEach(async ({ page }) => {
  6   |     await loginAsSuperAdmin(page);
  7   |   });
  8   |
  9   |   test('super admin dashboard stats render correctly', async ({ page }) => {
  10  |     await expect(page.getByRole('heading', { name: /platform dashboard/i })).toBeVisible();
  11  |     await expect(page.getByRole('heading', { name: /institute health/i })).toBeVisible();
  12  |     await expect(page.getByRole('heading', { name: /platform growth/i })).toBeVisible();
